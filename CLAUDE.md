...
The loss combines both with sequence-level truncated IS (threshold c=8.0 in the paper): `L = −E[min(π/μ, c)·(R − b*)·log π]`, optimized with AdamW. The paper PDF is in `paper/`.

VCPO is implemented for the **Megatron backend**. Environment per README: Megatron-Core 0.13.1 + vLLM 0.11.0.

Local work on top of the VCPO base (see `git log` since `ff7c8aa`): message-queue and cancel-queue checkpointing, async tool-use loop with validation + "simple-tir" tool parser, code sandbox server, dataset-source-aware reward calculation, configurable batch size per rollout DP replica. The `actor_entropy_logging` branch additionally has actor-entropy logging and a fix for logging across multiple PPO epochs (not merged to `main`).
...
# Plot experiment results from TensorBoard logs in cloud-logs/
python scripts/plot_vcpo_comparison.py [--max-steps N] [--runs subdir:"Label" ...]
python scripts/plot_val_vs_time.py
python scripts/plot_vcpo_single_run.py
```
...
Newer script filenames encode the setup: `vcpo_k=10_6gpu_math_3+3_ppo_epochs=2.sh` means k=10 staleness, 6 GPUs split 3 rollout + 3 training, MATH dataset, 2 PPO epochs. Key knobs (`MODEL_PATH`, `TRAIN_FILE`, `TEST_FILE`, `n_gpus_rollout`, `NGPUS_PER_NODE`, `CUDA_VISIBLE_DEVICES`) are env-overridable.

## Experiment artifacts and analysis notes (untracked, repo root)

- `cloud-logs/` — TensorBoard logs of completed runs (`main_ppo` colocated-sync baseline, `async_k-0`, `async_k-10`, `vcpo_dapo17k-aime24_k-*`), consumed by the `scripts/plot_*.py` tools. `scripts/plot_vcpo_comparison.py`'s docstring documents the timing semantics used to compare sync vs async wall-clock fairly (why `cumsum(timing_s/step)` is used instead of the logged `fully_async/timing/cumulative_training_time`, which understates async time by ~12%).
- Root-level `.md`/text notes are investigation writeups from past debugging sessions: `DIFF_synchronous_vs_vcpo_k12.md` (exact config delta between the k=0 baseline and VCPO scripts), `DIFF_vcpo_vs_vaco.md`, `DIFF_datasets_aime_dapo.md`, `ENTROPY_COLLAPSE_colocated_sync.md` (the colocated-sync step-420 collapse analysis), `VCPO_DAPO17K_SLOW_LEARNING_DISCUSSION.md`, `zero-utilization` (why rollout GPUs idle during validation reward scoring), `implementation.txt`, `diff_verl.txt` (diff vs upstream veRL). Read these before re-investigating a symptom they already cover.
