...
- KL to a reference policy (`actor.use_kl_loss`, reference = the initial model) works on both fully-async paths: the replay path computes reference log-probs once per group when it is first composed (`FullyAsyncTrainer._ensure_ref_log_probs`, cached in the group's `full_batch`), the per-trajectory actor path scales the KL / entropy terms by the advantage-free `aux_loss_multiplier` (never by the folded advantage), and `async_training.kl_ref_reset_interval` re-anchors the reference at the current policy in-process (`DetachActorWorker.reset_ref_to_actor`, Megatron only, optimizer state kept). Incompatible with OPOB. Arm env knobs: `use_kl_loss`, `kl_loss_coef`, `kl_loss_type`, `kl_ref_reset_interval`, `ref_param_offload`.
