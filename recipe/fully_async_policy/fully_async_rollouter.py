...
class FullyAsyncRollouter(FullyAsyncRayPPOTrainer):
    ...
    def __init__(
        ...
        # Concurrency ramp (async_training.concurrency_ramp, default off): per-engine in-flight caps
        # for the warm-up stages, keyed on groups DELIVERED to the queue (total_generated_samples).
        # Stage 0 lasts until the trainer's first mini-batch is delivered, stage i until
        # first + i x mini_size; afterwards the cap is n_engines x bsz_per_dp_rank as always. Few
        # sequences in flight at start = single-wave decode = the first update arrives in ~2 min
        # instead of the ~7 min a 165-group burst needs; the later stages over-supply the trainer.
    ...
    def _concurrency_cap(self) -> int:
        """Current in-flight group cap: the ramp stage for the groups delivered so far (see
