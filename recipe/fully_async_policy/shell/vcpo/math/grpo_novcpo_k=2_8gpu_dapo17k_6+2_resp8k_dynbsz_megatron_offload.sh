...
# Vanilla fully-async GRPO, throughput-oriented variant of
# grpo_novcpo_k=2_8gpu_dapo17k_4+4_resp8k.sh: 6 rollout + 2 trainer GPUs,
# trainer tp=1/dp=2, dynamic bsz. Optimizer fully offloaded to CPU
# (HybridDeviceOptimizer, exact fp32 updates) — REQUIRED at tp=1/dp=2 on 80 GB
# (GPU-resident exact optimizer needs ~82 GB static). Do NOT use
# use_precision_aware_optimizer without optimizer_cpu_offload: no master
# weights on MCore 0.13.1 + TE 2.6, ~98% of lr=1e-6 updates round to zero
# (silent stall).
...
# Token budget 12288, NOT 2x max_model_len: at 20480 the tp=1 full-vocab fp32
# logits transient (~9.6 GiB) plus expandable segments starves NCCL's lazy
# communicator alloc in the grad-norm all-reduce (6+2 smoke, 2026-07-30).
# 12288 still fits one max-length sequence.
