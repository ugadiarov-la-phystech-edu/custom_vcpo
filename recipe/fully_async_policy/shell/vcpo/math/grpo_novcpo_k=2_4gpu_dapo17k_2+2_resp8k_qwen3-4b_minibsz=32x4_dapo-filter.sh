...
# Vanilla fully-async GRPO on the fork's pipeline — variant of
# grpo_novcpo_k=2_8gpu_dapo17k_4+4_resp8k.sh for a 4x H100 machine: Qwen3-4B,
# 2 rollout + 2 trainer GPUs.
#
# Mini-batch variant of grpo_novcpo_k=2_4gpu_dapo17k_2+2_resp8k_qwen3-4b.sh:
# ppo_mini_batch_size=32 x require_batches=4 (was 128 x 1). Identical data
# consumption and staleness budget per step, but 4 sequential AdamW updates
# per sync instead of 1 (standard PPO mini-batching, absorbed by the clip).
#
# Trainer parallelism: tp=1/dp=2. At ~4B the tp=1 static footprint (~40 GB)
# stays GPU-resident with room for micro-batch transients, so no offload — and
# dp=2 avoids TP all-reduce overhead. sequence_parallel needs tp>1.
#
# DAPO-FILTER ARM: elastic dynamic filtering ON
# (async_training.dynamic_filtering). The rollouter drops zero-advantage-
# variance groups (all-correct/all-wrong), but only while the queue already
# buffers >= min_buffered_batches full trainer batches, so a drop never delays
# a step. This 2+2 config is trainer-paced from step ~2, so filtering runs at
# near-full strength. Watch fully_async/count/filtered_degenerate_groups;
# compare vs the unfiltered B-32x4 baseline at equal cumulative_training_time.
#
...
# 6144 (was 8192): cuts step time ~25-30%, at the cost of truncating ~55-60%
# of responses and making AIME24 validation incomparable with 8k-cap runs.
...
# oversubscribes vLLM's KV scheduler (measured 32 vs 44: identical fill).
...
# 1.0 full batches buffered before drops: preserves the never-delay-a-step
# invariant; in this trainer-paced layout filtering is ~always on, yet auto-
# suspends during post-validation queue refills.
...
# while the queue holds no full next batch. serialize_validation /
# pause_generation_during_save freeze the pipeline during validation and saves
# so the clock and trajectory match a no-validation-no-save run.
