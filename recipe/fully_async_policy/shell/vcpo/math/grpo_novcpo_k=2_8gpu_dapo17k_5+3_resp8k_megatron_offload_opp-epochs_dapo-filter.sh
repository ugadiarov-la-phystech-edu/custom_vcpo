...
# grpo_novcpo_k=2_8gpu_dapo17k_5+3_resp8k_megatron_offload.sh (5 rollout +
# 3 trainer GPUs, Megatron backend, HDO full CPU offload, mini-batch 129,
# fixed micro-batching) plus the elastic-utilization mechanisms:
#   * OPPORTUNISTIC PPO EPOCHS ON: while the next batch is not yet complete in
#     the queue, the trainer replays extra shuffled PPO epochs over the current
#     129-group batch (up to opportunistic_max_extra_epochs per step). Active
#     exactly in this layout's early rollout-bound phase (trainer idle ~40% at
#     the start, decaying to ~0 by step ~85); dormant once the run balances.
#   * DAPO-STYLE DYNAMIC FILTERING ON: the rollouter scores each completed
#     group and drops zero-advantage-variance ones (all-correct/all-wrong) —
#     but only while the message queue already buffers >=
#     min_buffered_batches full trainer batches, so drops are only taken when
#     the regeneration they license is free. Mirror image of the epochs:
#     dormant while rollout-bound, active once the pipeline turns
#     trainer-bound. Dropped groups are logged as
#     count/filtered_degenerate_groups.
#   * serialize_validation=True / pause_generation_during_save=True:
...
# needs TP>1), mini-batch 129 so 129*16=2064 seqs divide by DP=3, HDO full
# CPU offload with bf16 master weights (do NOT swap for
# use_precision_aware_optimizer without optimizer_cpu_offload: silent stall,
# probe 2026-07-30). VCPO mechanisms stay off.
...
num_minibatches_per_update=4 # 4 AdamW steps of 33 groups per trainer step; required_samples=33*4=132 keeps the rollouter identical to the B-132 baseline
partial_rollout=True
use_rollout_log_probs=True

# ================= Dynamic filtering (DAPO-style, elastic) =================
# Drops are allowed only once >= min_buffered_batches full trainer batches sit
# in the queue: never delays a trainer step. In this layout the queue runs
# shallow during the early rollout-bound phase (filter dormant) and fills as
# generation speeds up (filter active exactly when replacement generation is
# free).
dynamic_filtering_enable=${dynamic_filtering_enable:-True}
min_buffered_batches=${min_buffered_batches:-1.0}

# ================= Opportunistic epochs / stop-the-world accounting =================
# Extra shuffled PPO epochs over the current batch while the next batch is
# still incomplete — fills the early rollout-bound trainer idle (~40% at
# start); auto-dormant once updates become the bottleneck.
