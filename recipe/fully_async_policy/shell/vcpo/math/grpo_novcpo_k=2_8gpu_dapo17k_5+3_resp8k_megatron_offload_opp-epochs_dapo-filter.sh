...
# grpo_novcpo_k=2_8gpu_dapo17k_5+3_resp8k_megatron_offload.sh (5 rollout + 3
# trainer GPUs, Megatron + HDO full CPU offload, fixed micro-batching) plus
# the elastic-utilization mechanisms:
#   * OPPORTUNISTIC PPO EPOCHS ON: extra shuffled epochs over the current
#     batch while the next one is incomplete (active in the early
#     rollout-bound phase, dormant once balanced).
#   * DAPO-STYLE DYNAMIC FILTERING ON: drop zero-advantage-variance groups,
#     only while the queue buffers >= min_buffered_batches full batches
#     (active exactly when replacement generation is free). Logged as
#     count/filtered_degenerate_groups.
#   * serialize_validation / pause_generation_during_save: stop-the-world
#     validation and saves, excluded from cumulative_training_time.
...
# Drops allowed only once >= min_buffered_batches full batches are queued:
# never delays a step; the filter is dormant while rollout-bound and active
# exactly when replacement generation is free.
...
# Extra shuffled PPO epochs while the next batch is incomplete — fills the
# early rollout-bound trainer idle, auto-dormant once updates bottleneck.
