...
# GPUs, Megatron backend:
#   * trainer tp=1 / dp=3 (2 does not divide 3; sequence_parallel off since it
#     needs TP>1 — same pattern as the 3+3 MATH scripts)
#   * fixed micro-batching like the 4+4 base (use_dynamic_bsz=False, 1 seq per
#     micro-batch) — NOT the dynamic-bsz packing of the 6+2 variant.
#   * mini-batch 129 prompts, not 128: 129*16=2064 sequences divides by DP=3
#     (the 32->33 trick from the 3+3 MATH scripts). total_rollout_steps scales
#     with it (500 steps x 129 prompts).
#   * optimizer fully offloaded to CPU (HybridDeviceOptimizer, exact fp32
#     updates via param_update_in_fp32) — the proven 3-trainer recipe. A
#     GPU-resident exact optimizer at dp=3 needs 16.4 (bf16 params) + 16.4
#     (bf16 grads) + 32.8 (fp32 master+moments /dp=3) ~= 66 GB static, leaving
#     too little for the tp=1 full-vocab logits/entropy transients; with full
#     offload the GPU holds only params+grads (~33 GB).
...
#     master weights at all and ~98% of lr=1e-6 updates round to zero (probe,
#     2026-07-30) -> silent stall.
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO k-${staleness_threshold} DAPO17K-AIME24 Qwen3-8B ${n_gpus_rollout}-${n_gpus_training} tp1dp3 hdo B-${train_prompt_mini_bsz} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"}
