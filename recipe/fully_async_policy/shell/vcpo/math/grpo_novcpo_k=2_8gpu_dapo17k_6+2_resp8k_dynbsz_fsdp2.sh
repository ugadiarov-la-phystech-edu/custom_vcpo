...
# FSDP2 pure DP=2, dynamic bsz, torchao _AdamW with bf16 stochastic rounding
# fully GPU-resident (~33 GB static). fsdp_config.model_dtype=bf16 and entropy
# chunking+checkpointing are REQUIRED (fp32 default quadruples static memory
# and OOMs; non-chunked entropy OOMs the backward). SR makes master-less bf16
# updates unbiased at lr=1e-6. Numerics differ from the Megatron runs — treat
# cross-backend comparisons as system-level. Uses the dp_actor deferred old-
# log-prob port. Requires torchao.
...
# 0.8, not 0.9: the FSDP-path engine process sits ~7-10 GB above the pool
# target, and margin is needed for sampler warmup and sync_rollout_weights
# broadcasts (0.9 and 0.85 both OOMed; 0.8 leaves ~5 GB free).
gpu_memory_utilization=0.8
# vLLM v1 warms the sampler with max_num_seqs dummy requests after filling the
# KV pool; 512 is far above the actual concurrency at these lengths.
max_num_seqs=512
