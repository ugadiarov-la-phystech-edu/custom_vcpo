#!/usr/bin/env bash
# Smoke test for grpo_novcpo_k=2_8gpu_dapo17k_6+2_resp8k_dynbsz_megatron_offload.sh
# (Megatron backend, tp=1/dp=2 trainer, HybridDeviceOptimizer full CPU offload,
# dynamic bsz, deferred old-log-prob) at FULL batch fidelity (rollout.n=16).
#
# Runs the real launch script end-to-end for only 2 optimizer steps with
# validation disabled, then asserts on the log:
#   1. the HDO optimizer override reached Megatron's OptimizerConfig
#      ("optimizer config after override" printout with optimizer_cpu_offload);
#   2. the deferred old-log-prob path engaged and the Megatron actor computed
#      IS weights in the backward pass (rollout_corr/ metrics in step lines);
#   3. both training steps completed — the first update exercises the unproven
#      memory region: tp=1 full-vocab logits/entropy at 20480-token packed
#      micro-batches (4x the logits area of any previous Megatron run);
#   4. no Traceback / CUDA OOM anywhere in the log.
# Also samples nvidia-smi and reports per-GPU peaks (expected: rollout ~75 GB
# at util 0.9; trainer ~33 GB static + logits/entropy transients).
#
# Usage (needs the training environment activated; all 8 GPUs free):
#   bash recipe/fully_async_policy/shell/vcpo/math/smoke_test_grpo_6+2_megatron_offload.sh
...
SCRIPT="recipe/fully_async_policy/shell/vcpo/math/grpo_novcpo_k=2_8gpu_dapo17k_6+2_resp8k_dynbsz_megatron_offload.sh"
LOG=${SMOKE_LOG:-logs/smoke_6+2_megatron_offload.log}
...
export exp_name="SMOKE-6+2-megatron-offload" # keep TB/ckpt junk out of real run dirs
...
# batch-tensor residency and full-batch update time are NOT measured at n=4 —
# run once with SMOKE_N=16 after the config passes, to get those numbers.
...
# --- 3. HDO optimizer override composed ------------------------------------
...
# --- 4. deferred old-log-prob path engaged ----------------------------------
...
# --- 5. both steps completed, actor computed IS weights ----------------------
...
echo
if [ "${fail}" -eq 0 ]; then
    echo "[smoke] PASS — Megatron 6+2 + HDO offload + dynamic bsz + deferred old-log-prob: 2 steps trained without OOM"
else
