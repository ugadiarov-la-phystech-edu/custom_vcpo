...
# ================= Forward-lag reuse =================
# 2 optimizer updates per pulled batch: make_minibatch_iterator replays the
# batch ppo_epochs times. Advantages and rollout log-probs mu stay frozen
# across epochs; policy log-probs come from each epoch's live forward, so the
# IS ratio moves in epoch 2.
ppo_epochs=2
...
# rho_on measured from the k=10 DAPO17K run: on-policy (param version 0) staleness/ess_ratio
# was 0.002-0.011, run median ~0.004. At ~8.6k-token responses the vLLM/Megatron per-token
# mismatch dominates the seq-level IS ratios, so the on-policy ESS floor is ~0.005, not 1.0.
ess_scaling_base_ess_ratio=0.005
...
# ================= Logging =================
exp_name="VCPO k-${staleness_threshold} DAPO17K-AIME24 Qwen3-8B ${n_gpus_rollout}-${n_gpus_training} B-${train_prompt_mini_bsz} ppo-epochs-${ppo_epochs} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"
