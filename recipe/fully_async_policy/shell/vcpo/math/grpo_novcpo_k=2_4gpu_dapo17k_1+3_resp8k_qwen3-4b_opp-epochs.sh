...
# grpo_novcpo_k=2_4gpu_dapo17k_2+2_resp8k_qwen3-4b.sh rebalanced to 1 rollout +
# 3 trainer GPUs on the same 4x H100 machine. Same experiment identity (vanilla
# fully-async GRPO, k=2, Qwen3-4B, DAPO-17k, n=8, 6144 response cap, VCPO
# mechanisms off). Layout adaptations:
#   * trainer tp=1 / dp=3 (tp already 1 in the base; dp grows 2->3). Per-GPU
#     static footprint shrinks to ~32 GB (bf16 params 8 + bf16 grads 8 + fp32
#     master/AdamW 48/3 = 16), so the optimizer stays GPU-resident with even
#     more transient headroom than the 2+2 base — no offload needed.
#     sequence_parallel stays False (requires tp > 1).
#   * mini-batch 129 prompts, not 128: 129 = 3*43 divides by trainer DP=3 at
#     group level, so 129*n sequences split evenly for any n.
#     total_rollout_steps scales with it (500 steps x 129 prompts).
#   * single vLLM engine: bsz_per_dp_rank=32 keeps the per-engine concurrency
...
#     step, ~1 expected at these R/T). Stop-the-world validation and saves are
#     also on, so cumulative_training_time and the trajectory are exactly
#     comparable to a no-validation-no-save run. Set opportunistic_enable=False
#     at launch for the matching faithful baseline arm.
#
...
# (~40-50% of responses AT the cap). The 6k cap cuts step time ~25-30% at the
# cost of truncating ~55-60% of responses (scored wrong) and making AIME24
# validation incomparable with 8k-cap runs.
max_response_length=6144
max_num_batched_tokens=$((max_prompt_length + max_response_length))

# ================= Megatron Parallelism =================
train_tp=1 # tp=1/dp=3 on the 3 trainer GPUs (see header)
train_pp=1
train_cp=1
...
bsz_per_dp_rank=32 # Rollout Bsz; 1 engine -> 32 concurrent prompt-groups total
...
opportunistic_max_extra_epochs=${opportunistic_max_extra_epochs:-4}
...
# 2000 steps x 33 prompts = 66,000 prompts — equal-data horizon with the B-129
# variant (500 x 129 = 64,500) so curves are comparable at equal prompts, not
# equal steps.
total_rollout_steps=${total_rollout_steps:-$((2000 * num_minibatches_per_update * updates_per_param_sync * train_prompt_mini_bsz))}
epochs=10000000
# At 33-prompt steps, freq=20 restores the per-prompt cadence of the 128/129-
# prompt runs at freq=5 (~every 660 prompts). Validation runs on the single
# engine (~2x slower than 2+2) and both pauses freeze the pipeline under the
# stop-the-world flags above — denser cadence would eat most of the wall time.
