...
# + 3 trainer GPUs; same experiment identity (vanilla async GRPO, k=2,
# Qwen3-4B, DAPO-17k, n=8, 6144 cap, VCPO off). tp=1/dp=3 keeps the optimizer
# GPU-resident (~32 GB static); mini-batch 129 divides by DP=3; the single
# vLLM engine makes this layout heavily rollout-bound; dynamic bsz
# (16384-token budget) speeds updates. OPPORTUNISTIC PPO EPOCHS ON, plus stop-
# the-world validation/saves. opportunistic_enable=False gives the matching
# baseline arm.
#
...
# variant, so curves compare at equal prompts, not equal steps.
total_rollout_steps=${total_rollout_steps:-$((2000 * num_minibatches_per_update * updates_per_param_sync * train_prompt_mini_bsz))}
epochs=10000000
# freq=20 restores the per-prompt validation cadence of the 128/129-prompt
# runs at freq=5 (~every 660 prompts); denser would eat the wall time under
# the stop-the-world flags.
