...
# grpo_novcpo_k=2_8gpu_dapo17k_5+3_resp8k_fsdp2_dynbsz_ppo-epochs=2_B33x1.sh
# FSDP2 + dynamic-batch-size variant of the 5+3 B-33x1 arm
# (grpo_novcpo_k=2_..._5+3_..._megatron_offload_ppo-epochs=2_B33x1.sh).
# Same experiment design as the Megatron arm:
#   * 5 rollout + 3 trainer GPUs, ppo_mini_batch_size=33 (33*16=528 seqs,
#     528/DP=3 -> 176 per trainer rank), require_batches=1 (B-33x1),
...
#     trainer step, round(2*1)=2 AdamW updates — two shuffled passes over the
#     same 33 groups; second-epoch IS ratios recomputed against the current
#     policy per update (skip_recompute path — works on FSDP via the dp_actor
#     port, tests/workers/actor/test_skip_recompute_old_log_prob_on_cpu.py).
...
# What replaces the Megatron HDO trainer (mechanics from the two reference
# scripts grpo_novcpo_k=2_..._5+3_resp8k_fsdp2.sh and
# grpo_novcpo_k=2_..._6+2_resp8k_dynbsz_fsdp2.sh):
#   * trainer FSDP2 dp=3, verl's STANDARD precision recipe (from the 5+3
#     FSDP2 script): fp32 model params (= fp32 master, model_dtype left at
#     default) + plain AdamW with fp32 states, bf16 mixed-precision compute.
#     ~10.9 (fp32 params) + ~10.9 (grads) + ~21.9 (AdamW states) ~= 44 GB
#     static per trainer GPU — no stochastic rounding, no CPU offload;
#     numerics match the Megatron baselines (exact fp32 updates).
#   * use_dynamic_bsz=True (from the 6+2 dynbsz script): micro-batches packed
#     to a 20480-token budget (2x max_model_len) instead of 1 seq per
#     micro-batch — 1.5-3x faster updates on the fork's dynbsz smoke data.
#     NOTE: the 20480 budget was smoke-validated on the bf16 6+2 arm (~33 GB
#     static); this fp32 recipe holds ~44 GB, so the packed backward is ~11 GB
#     tighter. Entropy chunking+checkpointing, gradient checkpointing and the
#     trainer-scoped expandable segments (fsdp_workers.DetachActorWorker) are
#     what make it fit. If the first packed update OOMs, halve the budget:
#     ppo_max_token_len=10240 (= 1x max_model_len, exactly the fixed-path
#     per-micro-batch ceiling, so it cannot be worse than the Megatron arm).
...
bsz_per_dp_rank=33 # Rollout Bsz: in-flight prompt-groups per rollout replica (33*5 replicas = 165 concurrent)
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO k-${staleness_threshold} DAPO17K-AIME24 Qwen3-8B ${n_gpus_rollout}-${n_gpus_training} fsdp2 dynbsz B-${train_prompt_mini_bsz}x${num_minibatches_per_update} ppo-epochs-${ppo_epochs} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"}
