...
# grpo_novcpo_k=2_8gpu_dapo17k_4+4_resp8k_fsdp2_dynbsz_ppo-epochs=2_B33x1.sh
# 4+4-layout variant of the 5+3 FSDP2 dynbsz B-33x1 arm
# (grpo_novcpo_k=2_..._5+3_resp8k_fsdp2_dynbsz_ppo-epochs=2_B33x1.sh) with
# ONE change:
...
#     Side benefit: the fp32 recipe's static footprint shards down to
#     ~8.2 (fp32 params) + ~8.2 (grads) + ~16.4 (AdamW states) ~= 33 GB per
#     trainer GPU (vs ~44 GB at DP=3) — the SAME envelope the bf16 6+2 arm
#     smoke-validated the 20480-token dynbsz budget at, so the 5+3 fp32
#     sibling's OOM caveat is much softer here (fallback
#     ppo_max_token_len=10240 still available if needed).
# Everything else inherited from the 5+3 arm:
#   * trainer FSDP2 pure DP, verl's STANDARD precision recipe: fp32 model
#     params (= fp32 master, model_dtype left at default) + plain AdamW with
#     fp32 states, bf16 mixed-precision compute — no stochastic rounding, no
#     CPU offload; numerics match the Megatron baselines (exact fp32 updates).
#   * use_dynamic_bsz=True: micro-batches packed to a 20480-token budget
#     (2x max_model_len) — 1.5-3x faster updates than 1 seq per micro-batch.
#     Entropy chunking+checkpointing and gradient checkpointing kept.
...
# logits). At DP=4 the fp32 static is ~33 GB — the same envelope the bf16
# 6+2 arm validated this budget at. Fall back to 10240 if it OOMs.
