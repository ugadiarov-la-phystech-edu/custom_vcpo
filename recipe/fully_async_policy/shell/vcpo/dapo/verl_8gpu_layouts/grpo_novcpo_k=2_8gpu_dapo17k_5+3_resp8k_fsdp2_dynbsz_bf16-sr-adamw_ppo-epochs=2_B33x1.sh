...
# grpo_novcpo_k=2_8gpu_dapo17k_5+3_resp8k_fsdp2_dynbsz_bf16-sr-adamw_ppo-epochs=2_B33x1.sh
# bf16 + torchao stochastic-rounding AdamW variant of the fp32 FSDP2 dynbsz arm
# (grpo_novcpo_k=2_..._5+3_resp8k_fsdp2_dynbsz_ppo-epochs=2_B33x1.sh), swapping
# in the trainer precision recipe of the 6+2 dynbsz script
...
#     ~= 22 GB static per trainer GPU — HALF the fp32 recipe's ~44 GB. The
#     freed headroom is spent on a LARGER dynbsz packing budget: 30720
#     tokens (3x max_model_len) instead of the 20480 the 6+2 arm (~33 GB
#     static) smoke-validated; estimated peak ~60-65 GB, about what that arm
#     demonstrated. Fall back to ppo_max_token_len=20480 if it OOMs.
#   * model_dtype=bf16 is REQUIRED for this: verl builds the FSDP actor in
#     fp32 by default, which quadruples static memory AND makes
...
#     and the fp32 FSDP2 sibling is a third regime (fp32 master). Treat all
#     cross-recipe comparisons as system-level, not ablations.
#   * Requires torchao in the environment.
# Same experiment design as the Megatron/fp32 B-33x1 arms:
...
#   * use_dynamic_bsz=True: micro-batches packed to a 30720-token budget
#     (3x max_model_len) — 1.5-3x faster updates than 1 seq per micro-batch,
#     plus ~10-20% on top of the 2x budget from fewer FSDP2 param all-gather
#     rounds. Entropy chunking+checkpointing and gradient checkpointing kept.
...
# token budget per micro-batch: 3x max_model_len — the bf16 recipe's ~22 GB
# static (vs 33 GB on the smoke-validated 6+2 arm at 2x) affords the larger
# budget: estimated peak ~60-65 GB, about what the 6+2 arm demonstrated.
# Fewer micro-batches = fewer FSDP2 param all-gather rounds (~10-20% faster
# updates). Fall back to 20480 (2x, the validated budget) if it OOMs.
