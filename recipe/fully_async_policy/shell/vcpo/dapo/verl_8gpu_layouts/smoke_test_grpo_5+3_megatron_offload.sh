#!/usr/bin/env bash
# Smoke test for grpo_novcpo_k=2_8gpu_dapo17k_5+3_resp8k_megatron_offload.sh
# (Megatron backend, tp=1/dp=3 trainer, HybridDeviceOptimizer full CPU offload,
# fixed 1-seq micro-batches, mini-batch 129 prompts for DP=3 divisibility).
#
...
#   3. both training steps completed — first update exercises the dp=3 HDO
#      trainer (~33 GB static + tp=1 logits/entropy transients at fixed
#      10240-token micro-batches) and the 129-prompt mini-batch plumbing;
#   4. no Traceback / CUDA OOM anywhere in the log.
# Also samples nvidia-smi and reports per-GPU peaks (expected: 5 rollout GPUs
# ~75-77 GB at util 0.9; 3 trainer GPUs ~45-55 GB peak).
#
# Usage (needs the training environment activated; all 8 GPUs free):
#   bash recipe/fully_async_policy/shell/vcpo/math/smoke_test_grpo_5+3_megatron_offload.sh
...
export exp_name="SMOKE-5+3-megatron-offload" # keep TB/ckpt junk out of real run dirs
