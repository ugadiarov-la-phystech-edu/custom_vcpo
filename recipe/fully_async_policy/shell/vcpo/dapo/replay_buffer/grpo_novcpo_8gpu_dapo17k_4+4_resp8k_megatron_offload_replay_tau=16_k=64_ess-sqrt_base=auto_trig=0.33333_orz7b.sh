...
# 4+4-layout variant of
#   ..._dapo17k_5+3_resp8k_megatron_offload_replay_tau=16_k=64_ess-sqrt_base=auto_trig=0.33333_orz7b.sh
# Every setting below is byte-identical to that arm except the GPU split
# (n_gpus_rollout 5 -> 4, so 4 vLLM replicas + 4 trainer DP ranks), the
# comments that describe the layout, and the experiment name (the 5-3/tp1dp3
# tags become 4-4/tp1dp4). See "LAYOUT" below for why. Everything the 5+3 ORZ
# header says about the model, the prompts, the tag-aware reward and the
# validation sampling still applies verbatim.
#
...
# LAYOUT (the one deliberate change vs the 5+3 twin). The 5+3 ORZ run was
# generation-surplus-bound: the rollouter produced ~45 group arrivals per
# update against 33 consumed, so a standing backlog of aging groups formed and
# ~1/3 of generated groups died unseen at the k=64 eviction horizon
# (evicted_unseen_cum ~3,930 by step ~275). n_gpus_rollout=4 flips the layout
# to 4+4: generation drops x4/5 while the DP=4 trainer updates ~x3/4 faster,
# so arrivals/update drop to ~27 < 33 consumed -- the buffer drains instead of
# growing, no privileged backlog of aging groups can form, and evicted-unseen
# waste should go to ~0. Watch replay/minibatch_staleness_mean,
# replay/evicted_unseen_cum and rollout_corr/kl in the first 50 updates.
#
...
# and no trust_remote_code. tp=1/pp=1 with dp=4 here: 28 layers and 4 KV heads divide
...
# needs TP>1), 33*16=528 seqs divide by DP=4 (528/4=132), HDO full CPU offload with bf16
...
train_tp=1 # pure DP across the 4 trainer GPUs (no TP comm)
train_pp=1
train_cp=1
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO replay tau-${replay_tau} k-${replay_staleness_threshold} rmb-${replay_requires_mini_batches} ess-${ess_rule}-base-${ess_base_tag} DAPO17K-AIME24 ORZ-7B ${n_gpus_rollout}-${n_gpus_training} tp1dp4 hdo B-${train_prompt_mini_bsz} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"}
