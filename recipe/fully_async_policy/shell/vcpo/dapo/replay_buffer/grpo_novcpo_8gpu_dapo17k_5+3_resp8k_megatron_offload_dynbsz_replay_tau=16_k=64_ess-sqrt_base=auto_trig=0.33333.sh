...
# DYNAMIC-BATCH-SIZE variant of the TRIGGERED ESS-braked replay arm
# (grpo_novcpo_..._replay_tau=16_k=64_ess-sqrt_base=auto_trig=0.33333.sh),
# with ONE deliberate change on top of it:
...
#     quantity, better numerics (no fp32-exp censoring, ratio floored at
#     1/B). Per-traj grad-norm diagnostics and per-token record lists are
#     mbs=1-only and stay empty here. OPOB remains incompatible with dynbsz.
#     Token budget 20480 = 2x max seq len (conservative first flight — the
#     mbs=1 arm's activation profile does not transfer; raise after a smoke
#     run's perf/max_memory_* readings, cf. the fsdp2 arm's 25600).
# The parent arm's rationale (unchanged):
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO replay tau-${replay_tau} k-${replay_staleness_threshold} rmb-${replay_requires_mini_batches} ess-${ess_rule}-base-${ess_base_tag} DAPO17K-AIME24 Qwen3-8B ${n_gpus_rollout}-${n_gpus_training} tp1dp3 hdo dynbsz B-${train_prompt_mini_bsz} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"}
