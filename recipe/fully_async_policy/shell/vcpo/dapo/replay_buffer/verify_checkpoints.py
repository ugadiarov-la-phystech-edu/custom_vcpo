...
Checks what `save_contents=['hf_model']` is supposed to produce, without loading a full model:
  ...
  global_step_N/
    ...
    timing_state.json    cumulative timing totals + first-sample / save datetimes
    data.pt              the rollouter's dataloader state
  latest_checkpointed_iteration.txt

Ported from the baselines_main-ppo branch's baseline/verify_checkpoints.py; the timing_state.json
layout differs on this branch (FullyAsyncTrainer._save_timing_state): the datetime anchors are
``first_sample_datetime`` (null until the rollouter's first sample has been seen, so the very first
checkpoint may legitimately carry null) and ``checkpoint_saved_datetime``, both formatted
...
TIMING_DATETIME_FORMAT = "%Y-%m-%d %H:%M:%S"


def _parse_timing_datetime(value):
    """Parse a timing_state.json datetime anchor; None stays None (allowed for first_sample_datetime)."""
    if value is None:
        return None
    return datetime.strptime(value, TIMING_DATETIME_FORMAT)
...
def verify_checkpoint(step_dir, report, base_state=None, expect_dtype=None, expect_resumable=None):
    ...
    if "checkpoint_saved_datetime" in timing:
        try:
            report.check(
                _parse_timing_datetime(timing["checkpoint_saved_datetime"]) is not None,
                f"{name}: checkpoint_saved_datetime={timing['checkpoint_saved_datetime']} present",
            )
        except (TypeError, ValueError) as exc:
            report.check(False, f"{name}: checkpoint_saved_datetime unparseable: {exc}")
    if "first_sample_datetime" in timing:
        try:
            _parse_timing_datetime(timing["first_sample_datetime"])
            report.check(True, f"{name}: first_sample_datetime={timing['first_sample_datetime']}")
        except (TypeError, ValueError) as exc:
            report.check(False, f"{name}: first_sample_datetime unparseable: {exc}")
    return timing
...
def main():
    ...
    if len(complete) > 1:
        # first_sample_datetime is null until the trainer has seen the rollouter's first sample
        # (learned from the validation stream at the next sync), so nulls are skipped here.
        starts = {t["first_sample_datetime"] for _, t in complete if t.get("first_sample_datetime")}
        report.check(len(starts) <= 1, f"all checkpoints share one first_sample_datetime: {starts}")
        stamps = [_parse_timing_datetime(t["checkpoint_saved_datetime"]) for _, t in complete]
        report.check(stamps == sorted(stamps), f"checkpoint_saved_datetime increases with the step: {stamps}")
