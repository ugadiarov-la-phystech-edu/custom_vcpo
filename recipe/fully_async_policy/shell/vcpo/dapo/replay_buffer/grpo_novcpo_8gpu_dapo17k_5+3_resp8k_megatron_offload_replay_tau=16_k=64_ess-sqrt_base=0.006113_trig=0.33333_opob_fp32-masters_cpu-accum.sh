...
#SBATCH --job-name=grpo-novcpo-replay-ess-opob-cpuaccum

# OPOB (VCPO optimal off-policy baseline) replay arm, 5+3 layout, fp32 master
# weights, host-resident OPOB accumulators: identical to
...
# deadband, 5 rollout + 3 trainer GPUs, B=33x16) except for OPOB, where its
# accumulators live, and the optimizer's master-weight precision:
...
#     ~58 GB + 30.5 -> ~88 GB). This arm keeps 5 rollout GPUs AND the textbook
#     optimizer numerics (fp32 master weights, fp32 Adam) by moving everything
#     that does not have to be on the GPU to the host:
#       - Adam moments: HDO (optimizer_cpu_offload=True, fraction 1.0, torch
#         AdamW on the CPU in fp32). The fp32 master shard (30.5/3 = 10.2 GiB)
#         and the fp32 gradient copy it needs at the step (10.2 GiB) stay on
#         the GPU. NO precision-aware / bf16-master deviation (unlike the 4+4
#         arm and the 5+3 twin).
#       - OPOB accumulators: grad_baselining.accum_device=cpu keeps G_R and G_S
#         in pinned host memory, in fp32 (accum_dtype=float32; 2 x 30.5 GiB per
#         trainer rank). Each trajectory's gradient (15.3 GiB bf16) is copied
#         d2h ONCE into a pinned staging buffer and added into both
#         accumulators on the CPU (accum_cpu_threads torch threads); the
#         -b* G_S move at the group close and the copy of the final G_R - b* G_S
#         back into Megatron's buffer at the step happen there too. Zero extra
#         GPU memory for OPOB, and fp32 accumulation over all 176 trajectories.
#       - The per-trajectory prepare_grads() inside _compute_grad_norms is
#         skipped (bf16 training has no grad scaler, so it only materialized
#         the optimizer's fp32 gradient copy); that copy now exists only at
#         the step, when the activations are already freed.
#     Estimate per trainer GPU: 15.3 params + 15.3 main grad buffer + 10.2 fp32
#     master shard + ~16 activations (10K tokens, full recompute) + ~6
#     non-torch ~= 63 GB during trajectories; ~57 + 10.2 (grad copy) at the
#     step. Host RAM per rank: 61 (accumulators) + 15.3 (staging) + ~20 (Adam
#     moments) ~= 97 GB -> ~290 GB for the 3 ranks (plus the replay buffer on
#     the driver).
...
# Base-script notes that still apply: HDO full CPU offload of the Adam moments,
# but with FP32 master weights (the twin's use_precision_aware_optimizer /
# main_params_dtype=bfloat16 overrides are removed); bf16 grad buffers
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO replay tau-${replay_tau} k-${replay_staleness_threshold} rmb-${replay_requires_mini_batches} ess-${ess_rule}-base-${ess_base_tag} opob-${grad_baselining_scope}-w2-normstd-${grad_baselining_accum_device}accum-${grad_baselining_accum_dtype} fp32-masters DAPO17K-AIME24 Qwen3-8B ${n_gpus_rollout}-${n_gpus_training} tp${train_tp}dp$((n_gpus_training / train_tp)) hdo B-${train_prompt_mini_bsz} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd seed-${SEED}"}
