...
#SBATCH --job-name=grpo-novcpo-replay-ess-opob

# OPOB (VCPO optimal off-policy baseline) replay arm: identical to
...
# deadband) except for OPOB and the trainer layout OPOB needs:
...
#     main buffer for the isolated g_i + accum G_R + score G_S). The twin's
#     5+3 / TP=1 layout is buffer-free at ~58 GB per trainer GPU; OPOB adds
#     2 x 15.26 GiB bf16 -> ~88 GB = OOM on 80 GB. Hence 4+4 with TP=2 /
#     DP=2 (the layout of the vcpo_k=10 4+4 arms): every per-rank tensor
#     halves — ~7.6 params + 3 x 7.6 buffers + ~3.8 bf16 master shard +
#     ~8 activations (full recompute, 10K tokens) + ~6 non-torch ~= 48 GB.
#     Per-traj grad norms are TP-aware (reduced over the TP group in
#     _compute_grad_norms); the DP all-reduce stays deferred to the step.
#     Costs: one fewer rollout GPU (the twin is trainer-bound, rollout idle
#     < 1%) and TP comm; the paper reports +19% step time for OPOB itself.
#   * train_prompt_mini_bsz=32 (512 seqs) instead of 33: group-scope OPOB
...
#     prompt count must divide by DP=2 -> 16 groups / 256 seqs per rank.
...
#     3.23 of B=528). The ESS ratio is DP-all-reduced (ESS / B over the whole
#     mini-batch), so B 528 -> 512 shifts rho_on by ~3% — within calibration
#     noise. ess_base=null re-enables auto-calibration from the first update.
...
#     buffer-free twin dropped (the ~20% figure of the pre-buffer-free code
#     applies again, on top of TP=2 comm).
...
#     while the buffer holds < 1*32 = 32 groups.
#   * async_training.staleness_threshold=64 aligns the rollouter's generation
#     quota with the eviction horizon (32*(64+1)=2080 groups licensed; in
#     practice a stall backstop — concurrency caps at 4*32=128 in-flight).
...
# (grad_reduce_in_fp32=False) — with three buffer copies this is what keeps
# the TP=2 footprint under 80 GB. Layout here: trainer tp=2/dp=2 with
# sequence_parallel=True, 32*16=512 seqs divide by DP=2. OPOB ON.
...
# 4 rollout + 4 trainer GPUs: OPOB's three grad-buffer copies need TP=2 on
# 80 GB cards (see header); 4 trainer GPUs = TP2 x DP2.
...
train_tp=2 # halves every per-rank param/grad-buffer tensor: 3 buffer copies (OPOB) fit in 80 GB
train_pp=1
train_cp=1
sequence_parallel=True # TP>1: shard the per-token activations too (4+4 precedent scripts)
...
train_prompt_bsz=0
gen_prompt_bsz=1
# 32*16=512 seqs. Group-scope OPOB needs whole prompt-groups per DP rank, so the
# PROMPT count (not just mini*n) must divide by trainer DP=2: 16 groups = 256 seqs per rank.
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO replay tau-${replay_tau} k-${replay_staleness_threshold} rmb-${replay_requires_mini_batches} ess-${ess_rule}-base-${ess_base_tag} opob-${grad_baselining_scope}-w2-normstd DAPO17K-AIME24 Qwen3-8B ${n_gpus_rollout}-${n_gpus_training} tp${train_tp}dp$((n_gpus_training / train_tp)) hdo B-${train_prompt_mini_bsz} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd seed-${SEED}"}
