...
#SBATCH --job-name=grpo-novcpo-replay-ess-trig-openpangu7b

# openPangu-Embedded-7B variant of
#   ..._replay_tau=16_k=64_ess-sqrt_base=auto_trig=0.33333.sh
# Model + trust_remote_code handling taken from the FSDP2 openPangu arm
# (grpo_novcpo_..._fsdp2_openpangu7b_replay_..., branch
# replay_buffer_vcpo_ess_threshold_fsdp2_min-ess_cppo): MODEL_PATH points at
# the LOCAL RE-ALIASED checkpoint - openPangu ships as a trust_remote_code
# PanguEmbeddedForCausalLM that neither stock transformers AutoModel nor
# vLLM 0.11.0 can build, so scripts/realias_openpangu_to_llama.py rewrites
# config.json to LlamaForCausalLM (the architecture is llama-compatible;
# attention bias folds into the single bias flag). The re-aliased dir keeps
# the tokenizer auto_map, so the TOKENIZER is still PanguTokenizer via
# trust_remote_code=True (data.trust_remote_code feeds the dataloader
# tokenizer; actor_rollout_ref.model.trust_remote_code feeds HFModelConfig -
# agent-loop tokenizer, weight load, vLLM). Build the checkpoint with:
...
# feeds both data.trust_remote_code (dataloader tokenizer) and
# actor_rollout_ref.model.trust_remote_code (HFModelConfig): the re-aliased
# checkpoint still loads its PanguTokenizer through the auto_map
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO replay tau-${replay_tau} k-${replay_staleness_threshold} rmb-${replay_requires_mini_batches} ess-${ess_rule}-base-${ess_base_tag} DAPO17K-AIME24 openPangu-7B ${n_gpus_rollout}-${n_gpus_training} tp1dp3 hdo B-${train_prompt_mini_bsz} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"}
