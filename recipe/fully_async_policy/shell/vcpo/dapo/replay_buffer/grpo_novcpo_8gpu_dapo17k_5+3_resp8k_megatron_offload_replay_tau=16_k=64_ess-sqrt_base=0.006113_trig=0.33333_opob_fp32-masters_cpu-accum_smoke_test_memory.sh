#!/usr/bin/env bash
# Fast memory/error smoke for the 5+3 OPOB replay arm with fp32 master weights
# and host-resident OPOB accumulators
# (grpo_novcpo_8gpu_dapo17k_5+3_resp8k_megatron_offload_replay_tau=16_k=64_ess-sqrt_base=0.006113_trig=0.33333_opob_fp32-masters_cpu-accum.sh).
#
...
# fp32-master HDO optimizer: per trainer GPU the params, Megatron's main grad
# buffer, the fp32 master shard, the 10240-token activations and NCCL context
# (~63 GB estimated during trajectories, ~67 GB at the step with the fp32
# gradient copy); per trainer rank ~97 GB of host RAM (two fp32 accumulators,
...
#   bash "recipe/fully_async_policy/shell/vcpo/dapo/replay_buffer/grpo_novcpo_8gpu_dapo17k_5+3_resp8k_megatron_offload_replay_tau=16_k=64_ess-sqrt_base=0.006113_trig=0.33333_opob_fp32-masters_cpu-accum_smoke_test_memory.sh"
...
SCRIPT="recipe/fully_async_policy/shell/vcpo/dapo/replay_buffer/grpo_novcpo_8gpu_dapo17k_5+3_resp8k_megatron_offload_replay_tau=16_k=64_ess-sqrt_base=0.006113_trig=0.33333_opob_fp32-masters_cpu-accum.sh"
LOG=${SMOKE_LOG:-logs/smoke_replay_opob_cpu-accum_5+3.log}
...
# checkpoints.
export exp_name="SMOKE-replay-opob-cpu-accum-5+3"
...
echo "[smoke] launching OPOB 5+3/TP=1 cpu-accum fp32-masters replay run in background; waiting for ${WANT_UPDATES} updates (no timeout); log: ${LOG}"
bash "${SCRIPT}" > "${LOG}" 2>&1 &
RUN_PID=$!
...
# --- 1. crashes (the main check: fp32 masters + main buffer + activations at TP=1) ---
if grep -aqE "CUDA out of memory" "${LOG}"; then
    bad "CUDA OOM — the fp32-master 5+3 layout does NOT fit this configuration:"
...
# Sanity on the optimizer: the run must not have applied the bf16-master deviation.
if grep -aq "main_params_dtype': torch.bfloat16" "${LOG}"; then
    bad "optimizer config shows main_params_dtype=bfloat16 — the fp32-master override set did not land"
else
    note "optimizer keeps fp32 master weights (no main_params_dtype=bfloat16 in the resolved optimizer config)"
fi
...
if [ -s "${MEMLOG}" ]; then
    ...
    if [ "${trainer_peak}" -gt 79000 ]; then
        ...
        note "trainer-GPU peak ${trainer_peak} MiB — headroom OK (estimate was ~63-67 GB)"
    fi
fi
...
echo
if [ "${fail}" -eq 0 ]; then
    echo "[smoke] PASS — single-backward OPOB with host-resident fp32 accumulators + fp32 masters survived on the real model at 5+3/TP=1; opob/* and brake metrics flowing"
else
