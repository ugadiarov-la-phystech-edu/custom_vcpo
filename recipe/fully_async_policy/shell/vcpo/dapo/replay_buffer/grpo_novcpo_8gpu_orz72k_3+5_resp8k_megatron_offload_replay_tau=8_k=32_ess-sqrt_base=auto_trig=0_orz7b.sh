...
#SBATCH --job-name=grpo-novcpo-replay-ess-3p5-trig0-orz72k-tau8-k32
...
# update runs at full nominal lr. ess_trigger=0 here: ess_ratio/base is
# structurally positive (log-space ESS floored at 1/B, NaN guarded), so
# "ratio >= 0" holds on every mini-batch and the brake NEVER engages —
# this is the UNBRAKED control arm of the trig=0.33333 twin. Every step
# runs at full nominal lr; the ESS measurement, the auto-base capture and
# the actor/ess_scaled_lr logging (constant lr) all still run, keeping the
# metric plumbing identical across arms. null = legacy (engage whenever
# ratio < 1); 0.33333 = the braked twin's deadband at base/3.
ess_trigger=${ess_trigger:-0}
