...
#SBATCH --job-name=grpo-novcpo-replay-ess-3p5-trig0
...
# AceReason-Math (filtered): 41091 rows, verified 2026-08-30 on the parquet
# itself to be format-identical to dapo-math-17k — single user message with
# the same DAPO wrapper ("...last line ... Answer: ..."), data_source=
# math_dapo on every row, plain unboxed ground truths in reward_model
# (style rule-lighteval/MATH_v2), extra_info = index only. So the PROMPTS /
# ANSWER FORMAT analysis in the header carries over unchanged, and the
# tag-aware scorer needs no data_source-specific handling (it ignores
# data_source anyway). Token lengths (ORZ tokenizer, chat template applied):
# p50=213, p99=508, max~2161 — a handful of rows exceed
# max_prompt_length=2048 and are dropped by filter_overlong_prompts=True.
TRAIN_FILE=${TRAIN_FILE:-"/home/jovyan/datasets/math_datasets/acereason/acereason-math-filtered.parquet"}
...
# Two full epochs of AceReason-Math-filtered: 2 * 41091 = 82182 fed prompts
# (the DAPO arms' 66000 was ~1.6 epochs of this set). Fed prompts, not kept
# groups: filtering shortens the effective trained horizon proportionally.
total_rollout_steps=${total_rollout_steps:-82182}
epochs=10000000
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO replay tau-${replay_tau} k-${replay_staleness_threshold} rmb-${replay_requires_mini_batches} ess-${ess_rule}-base-${ess_base_tag} ACEREASON-AIME24 ORZ-7B ${n_gpus_rollout}-${n_gpus_training} tp1dp5 hdo B-${train_prompt_mini_bsz} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"}
