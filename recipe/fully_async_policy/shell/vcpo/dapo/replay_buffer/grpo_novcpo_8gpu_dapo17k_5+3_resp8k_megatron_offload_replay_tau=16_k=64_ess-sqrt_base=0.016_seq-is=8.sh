...
# DEADBAND ESS-braked replay arm with VCPO's original IS correction. Identical
# to grpo_novcpo_..._replay_tau=16_k=64_ess-sqrt_base=auto.sh (trainer-side
# replay buffer, tau=16, eviction k=64, rmb=1, sync after every update, DAPO
# insertion gate, frozen advantages / behavior log-probs, per-traj ESS-scaled
# updates) except for two deliberate changes:
#   1. base_ess_ratio=0.016 (deadband) instead of auto-calibration. The
...
#      bottom end: base = 0.5 * measured rho_on => healthy steps (ess
#      0.02-0.03) hit the min(1,.) cap and run at FULL lr, while a detaching
#      step still gets cut to ~0.1-0.3x. Goal: unbraked early slope, braked
#      collapse response. Override with ess_base=<v> (ess_base=null restores
#      auto-calibration; the exp name tag follows).
#   2. rollout_is=sequence, threshold 8.0 (the original VCPO correction)
#      instead of the token-level 2.0 used by all previous replay arms.
#      Sequence-level truncation min(seq_ratio, 8.0) is what the paper's
#      ESS brake was validated with, and the c=8 cap bounds exactly the
#      compounded seq-level detachment that drove the unbraked runaway
#      (healthy seq ratios ~1-30; detached ones in the thousands).
#      rollout_rs stays null: the paper found rejection/masking brittle
#      (can mask nearly everything at high divergence).
...
# Original-VCPO sequence-level truncated IS against the *cached* behavior
# log-probs (frozen at insertion): w = min(prod_t ratio_t, 8.0) per sequence
# (paper's c=8.0). Bounds compounded seq-level detachment directly, unlike
# the token-2.0 truncation of the earlier replay arms. rollout_rs stays null
# (paper: seq-level masking is brittle). use_policy_gradient=False is the
