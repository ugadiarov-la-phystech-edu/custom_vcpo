...
# MIN-ESS-braked DYNAMIC-BATCH-SIZE Megatron replay arm: the ESS brake is a
# floor detector — brake (lr * ess_lr_scale) only when the mini-batch's
# global ESS is <= min_ess (1.1) effective samples, i.e. within 10% of the
# structural ESS = 1 floor a single dominant sequence produces; all other
# steps run at FULL nominal lr. Replaces the auto-captured on-policy base +
# base/3 trigger + sqrt rule of the former
# ..._dynbsz_replay_tau=16_k=64_ess-sqrt_base=auto_trig=0.33333.sh (renamed
# to this script): the captured base was a one-mini-batch lottery draw
# (CV 63% across seeds on the fsdp2 backend) while the raw ESS trace is
...
#     verl/workers/utils/ess.py), so the two arms' ESS traces are directly
...
# budget as 0.9 on an 80 GiB H100). The cap reaches only the trainer (DetachActorWorker)
# processes; the rollout engines are separate detach workers, so the 80 GiB value is faithful
# here (unlike the colocated sync arms). The knob is only READ here, never set. Tagged in
# exp_name so emulated runs never share a log dir with real ones.
emu_tag=""
