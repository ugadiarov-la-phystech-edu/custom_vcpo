...
#SBATCH --job-name=grpo-novcpo-replay-ess-3p5-trig0.33333-orz72k-tau4-k8

# FIX ARM for the diverged ORZ-72k trig=0 run (see
# ORZ7B_UNBRAKED_DIVERGENCE_INVESTIGATION.md). Based on
#   ..._orz72k_3+5_..._replay_tau=8_k=32_ess-sqrt_base=auto_trig=0_orz7b.sh
# with exactly two fixes applied:
#   A. ESS brake re-enabled: ess_trigger 0 -> 0.33333 (sqrt rule, auto base);
#   B. staleness budget cut: replay_tau 8 -> 4, replay/async staleness
#      threshold (k) 32 -> 8.
# Everything else (model, data, reward, layout, optimizer, schedule) is
# byte-identical to the diverged arm.
#
...
#
# 3+5 LAYOUT VARIANT of the 5+3 twin: n_gpus_rollout=3 / n_gpus_training=5.
# The trainer side goes from DP=3 to DP=5 (tp=pp=1 unchanged), so the
# rollout:consume balance shifts even further toward the trainer than 4+4 —
# arrivals/update drop to ~3/5 of the 5+3 rate while updates get faster.
# FORCED batch change: mini*n must divide by trainer DP; 33*16=528 does not
# divide by 5, so train_prompt_mini_bsz moves 33 -> 35 (560 seqs = 5*112;
# 35 groups = 5*7 per rank). Everything ESS/replay-related that scales with
# B follows automatically (B = 560, ESS floor ratio 1/560); comments below
# quoting 528/33 for the 5+3 twin are updated where they describe THIS run.
#
...
# eviction k=32, rmb=1, sync after every update, DAPO insertion gate, frozen
...
#     the k/tau ratio (and so the weight at the eviction edge) matches the
#     tau=16/k=64 twin; the horizon is halved, so the buffer retains every
#     kept group of the last 32 updates (~half the twin's group count and
#     driver RAM).
...
#     practice a stall backstop — concurrency caps at 3*35=105 in-flight).
...
# Base-script notes that still apply: trainer tp=1/dp=5 (sequence_parallel
...
# ORZ-72k (orz_math_72k_collection_extended, converted 2026-08-30 by
# scripts/convert_orz72k_to_verl_parquet.py): 47,981 rows in the dapo17k
# parquet SCHEMA but with ORZ-NATIVE prompts — each user message is ORZ's own
# inner instruction ("answer inside <answer></answer> ... extracted by the
# \boxed{} tag"), byte-identical to original ORZ training prompts once the
# tokenizer chat template adds the conversation layer (verified against
# their zero_setting_base.py jinja). Filters applied at conversion: ~23.9k
# exact duplicates (the raw "72k" is ~48k unique), 537 long-verbal + 17
# empty ground truths, 21 degenerate problems; decontaminated against
# aime-2024/2025 only (one aime-2026-overlapping problem deliberately kept).
# Ground truths: 69.3% plain numeric, 23.3% LaTeX — the LaTeX ones REQUIRE
# the tiered-equality scorer upgrade in orz_tag_aware_math.py (ORZ is_equiv
# + sympy tiers); with the old string-only equality ~quarter of the training
# signal turns into false negatives. Token lengths (chat template applied):
# p50=212, p99=446, max=1067 — nothing near max_prompt_length=2048.
TRAIN_FILE=${TRAIN_FILE:-"/home/jovyan/datasets/math_datasets/orz/orz-math-72k.parquet"}
# Two validation sets in ORZ-ORIGINAL prompt format (built 2026-08-30 by
# scripts/convert_val_parquets_to_orz_prompts.py from the DAPO-wrapper
# parquets — same 30 problems x 32 copies each, same ground truths, only the
# prompt wrapper swapped to ORZ's inner instruction):
...
# The fresh *_orz stamps keep these curves separate from every DAPO-wrapper
# run — do NOT overlay them on val-core/math_dapo curves of earlier arms:
# same problems, different prompt protocol.
...
train_tp=1 # pure DP across the 5 trainer GPUs (no TP comm)
train_pp=1
train_cp=1
...
train_prompt_bsz=0
gen_prompt_bsz=1
train_prompt_mini_bsz=${train_prompt_mini_bsz:-35} # 35*16=560 seqs; mini*n must divide by trainer DP=5 (560/5=112)
...
# update runs at full nominal lr. ess_trigger=0.33333 here — THE BRAKE IS
# BACK ON (fix A of ORZ7B_UNBRAKED_DIVERGENCE_INVESTIGATION.md). The trig=0
# unbraked twin diverged: seq-level ESS pinned at 1/560 from step ~5,
# ess/base ~0.01 for the whole run, and with the lr never scaled the drift
# compounded into an entropy explosion by step 250. sqrt(0.01) cuts the
# effective lr ~10x exactly in that regime; the brake is free while
# near-on-policy (ess/base > 0.33 held only in the first ~4 updates).
...
staleness_threshold=${staleness_threshold:-8.0}
...
# ================= Replay buffer =================
# tau=4 / k=8 — HALVED TWICE from the diverged tau=8/k=32 arm (fix B of
# ORZ7B_UNBRAKED_DIVERGENCE_INVESTIGATION.md). ORZ-7B's entropy ~0.06 makes
# 3000-token seq log-ratios explode under drift the Qwen arms shrug off:
# at tau=8/k=32 the minibatch staleness mean was 8-12 updates (max 30) and
# the unclipped seq-ESS was degenerate from step 5. tau=4/k=8 targets a
# staleness mean of ~2-3 updates; k/tau stays in the house 2-4 band. The
# doubling experiment (tau16/k64 vs tau8/k32) halved time-to-collapse, so
# staleness exposure is a first-order dial for this model.
replay_enable=${replay_enable:-True}
replay_tau=${replay_tau:-4}
replay_staleness_threshold=${replay_staleness_threshold:-8}
...
# Two full epochs of the deduplicated ORZ-72k set: 2 * 47981 = 95962 fed
# prompts. Fed prompts, not kept groups: filtering shortens the effective
# trained horizon proportionally.
total_rollout_steps=${total_rollout_steps:-95962}
epochs=10000000
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO replay tau-${replay_tau} k-${replay_staleness_threshold} rmb-${replay_requires_mini_batches} ess-${ess_rule}-base-${ess_base_tag} ORZ72K-AIME24ORZ ORZ-7B ${n_gpus_rollout}-${n_gpus_training} tp1dp5 hdo B-${train_prompt_mini_bsz} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"}
