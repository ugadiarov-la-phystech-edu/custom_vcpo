...
# DYNAMIC-BASE ESS-braked replay arm (new-cohort estimator) with the standard
# token-level IS correction. Identical to
# grpo_novcpo_..._replay_tau=16_k=64_ess-sqrt_base=0.016_tok-is=2.sh
...
#   * base_ess_ratio is estimated DYNAMICALLY instead of fixed at 0.016:
#     async_training.ess_base_estimator.mode=new_cohort tracks an EMA of the
#     fresh (not-yet-trained-on) cohort's pooled ESS — the freshest data the
#     async pipeline can deliver — so the brake measures REPLAY EXCESS
#     relative to a reference that follows response-length and policy-
#     sharpness drift, instead of a hand-picked constant. The first-update
#     capture still runs and seeds the estimator (warm start + default clamp
#     ceiling: never brake harder than the legacy auto-calibration would);
#     clamp_min=0.002 keeps the reference above the degenerate ESS floor
#     1/B ~= 1/528 so the brake can never become structurally unable to
#     engage. Fresh-cohort moments are pooled (>=64 seqs per EMA fold) with
#     the single largest weight winsorized (one heavy-tail outlier must not
#     crush the reference). A staleness-regression diagnostic
#     (replay/ess_base_rho_on_est, s->0 intercept) logs the strict
#     numerics-only rho_on next to the operating base; it never brakes.
#     Rationale: the fixed 0.016 deadband beat base=auto at every equal-ctt
#     checkpoint (+0.02-0.05 val), but 0.016 = 0.5 * rho_on was calibrated at
#     ~2k-token responses; rho_on decays roughly as exp(-sigma^2*L) as
#     responses grow, so a frozen deadband slowly turns into an over- or
#     under-brake. This arm answers whether a self-calibrating reference
#     matches or beats the hand-tuned deadband.
...
# Dynamic base: base_ess_ratio stays null (auto) and the new_cohort estimator
# supplies the evolving reference via meta_info["ess_base_override"] (EMA of
# the fresh cohort's pooled ESS; the first-update capture seeds it and is the
# default clamp ceiling). Set ess_base_mode=first_update to fall back to the
# legacy frozen auto-calibration, or ess_base=<v> for a fixed deadband.
ess_base=${ess_base:-null}
ess_use_clipped=False # ESS from unclipped ratios (paper): the brake must see what truncation hides
ess_base_mode=${ess_base_mode:-new_cohort}   # new_cohort | first_update
ess_clamp_min=${ess_clamp_min:-0.002}        # just above the degenerate ESS floor 1/B (B=528)
ess_clamp_max=${ess_clamp_max:-null}         # null = first-update capture ceiling
ess_len_corr=${ess_len_corr:-False}          # quote the reference at the minibatch's mean length
ess_base_tag=${ess_base}
[ "${ess_base_tag}" = "null" ] && ess_base_tag="auto"
if [ "${ess_base}" = "null" ] && [ "${ess_base_mode}" = "new_cohort" ]; then
    ess_base_tag="dyn-cohort"
    [ "${ess_len_corr}" = "True" ] && ess_base_tag="dyn-cohort-lencorr"
fi

# ================= IS / Rollout Correction =================
# Token-level truncated IS against the *cached* behavior log-probs (frozen at
# insertion): w_t = min(ratio_t, 2.0) per token — same as the deadband and
# unbraked replay arms, so this run differs from the 0.016 deadband arm ONLY
# in how the brake reference is obtained (dynamic estimator vs fixed).
...
python -m recipe.fully_async_policy.fully_async_main \
    ...
    async_training.ess_base_estimator.mode="${ess_base_mode}" \
    async_training.ess_base_estimator.clamp_min="${ess_clamp_min}" \
    async_training.ess_base_estimator.clamp_max="${ess_clamp_max}" \
    async_training.ess_base_estimator.length_correction="${ess_len_corr}" \
