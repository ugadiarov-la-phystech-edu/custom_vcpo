...
# grpo_novcpo_k=1_8gpu_dapo17k_6+2_resp8k_fsdp2_dynbsz_sr-adamw_ppo-epochs=2_B33x1_is-pg.sh
#
# THROUGHPUT FSDP2 arm: same data, same objective, same schedule and the same
# B-33x1 pull as ..._megatron_offload_ppo-epochs=2_B33x1_is-pg.sh, but 6 rollout +
# 2 trainer GPUs and the master-less bf16 recipe of the 6+2 dynbsz script on branch
# rollout-dapo. Its sibling ..._5+3_resp8k_fsdp2_... keeps exact fp32 numerics; this
# one buys generation throughput and update speed instead. See BACKEND MAPPING at the
# end of this header for what changed against the megatron arm.
#
...
#   * nothing is rotated away: ~16.4 GB per save for Qwen3-8B (bf16, see below),
#     ~200 saves over the full run is ~3.3 TB. Raise save_freq at launch if the disk
#     is tighter.
...
#
# MEMORY, 8.19B params on 2xH100-80GB with fsdp_size=-1 (bf16 params + bf16 moments):
# params 16.4/2 ~ 8.2 GB + grads ~ 8.2 GB + AdamW moments 32.8/2 ~ 16.4 GB ~ 33 GB
...
#
# WHAT THIS ARM DOES DIFFERENTLY (mirrors the 6+2 dynbsz script of rollout-dapo):
#   * 6 rollout + 2 trainer GPUs. 33*16=528 seqs still divides by DP=2 (264/rank).
#     Rollout concurrency: min(6 engines * 16, max_required_samples 33*(1+1)*1 = 66)
#     = 66 in-flight groups, i.e. the full k=1 staleness budget.
#   * fsdp_config.model_dtype=bf16 + torchao.optim._AdamW with
#     bf16_stochastic_round: master-less bf16 training, ~8.2 (sharded bf16 params)
#     + 8.2 (grads) + 16.4 (bf16 moments) ~= 33 GB static per trainer GPU at DP=2.
#     model_dtype=bf16 is REQUIRED, not cosmetic: verl builds the FSDP actor in fp32
#     by default (fsdp_workers.py), which quadruples static memory AND makes
#     stochastic rounding silently inert (SR only acts on bf16 params). Without SR a
#     deterministic bf16 update at lr=1e-6 rounds to zero for ~98% of weights.
#     Needs torchao in the environment (0.17.0 here).
#   * use_dynamic_bsz=True with a 20480-token micro-batch budget (2x max_model_len),
#     for the update and both log-prob passes: packs several sequences per
#     micro-batch instead of one, which is where the update speedup comes from.
#     Entropy chunking + checkpointing are REQUIRED at that budget (non-chunked
#     entropy materializes ~18 GB of fp32 logits-sized intermediates per
#     micro-batch at vocab 151936).
#   * NUMERICS DIFFER from the megatron and the 5+3 fsdp2 arms (exact fp32 masters
#     there): treat cross-arm comparisons as system-level, not ablations.
#   * checkpoints come out bf16 (~16.4 GB per save, same as the megatron arm) rather
#     than the 5+3 fsdp2 arm's fp32 ~33 GB: get_fsdp_full_state_dict returns the FSDP
#     param dtype and save_pretrained writes it unchanged.
#
# ACCEPTED DIVERGENCES FROM THE MEGATRON ARM (same objective, different numerics and
# artifacts):
#   * optimizer moments are bf16 here (master-less + stochastic rounding).
...
# ================= FSDP2 Parallelism =================
# Pure data parallelism over the 2 trainer GPUs: FSDP2 shards params, grads and
...
param_offload=False
optimizer_offload=False # first fallback if the trainer OOMs (frees ~16 GB/GPU)
# bf16 params: without this verl builds the actor in fp32 and stochastic rounding
# becomes inert. See the header.
...
micro_bsz_per_gpu=1 # ignored under use_dynamic_bsz, kept for the config's validation
use_dynamic_bsz=True
# token budget per packed micro-batch: 2x max_model_len (conservative - the length
# distribution drifts during training and FSDP materializes full-vocab logits)
ppo_max_token_len=$((2 * (max_prompt_length + max_response_length)))
...
# ~16.4 GB per save for Qwen3-8B (bf16 on this arm), nothing is rotated away: ~200 saves is ~3.3 TB,
...
# ================= Logging =================
exp_name=${exp_name:-"GRPO-noVCPO is-pg k-${staleness_threshold} DAPO17K-AIME24 Qwen3-8B ${n_gpus_rollout}-${n_gpus_training} fsdp2 dynbsz sr-adamw B-${train_prompt_mini_bsz}x${num_minibatches_per_update} ppo-epochs-${ppo_epochs} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"}
