...
#SBATCH --job-name=main-ppo-sync-orz72k-grpo-B128xn32-mini32-orz7b

# SYNCHRONOUS ORZ-continuation arm, QWEN-GEOMETRY VARIANT WITH n=32 SAMPLES PER
# PROMPT: verl.trainer.main_ppo (the plain colocated hybrid-engine trainer, NOT
# recipe/fully_async_policy), GRPO without a critic, continuing Open-Reasoner-
# Zero-7B on ITS OWN RL training set (orz-math-72k) with ORZ prompts.
#
# This is main_ppo_sync_8gpu_orz72k_grpo_B128xn16_mini32_orz7b.sh with ONE knob
# changed: n_resp_per_prompt 16 -> 32 (twice the GRPO group size; every other
# setting, including the four Qwen-derived knobs below, is identical). The n=16
# arm trained FLAT for 540 updates (AIME-24 0.15-0.18 around the 0.168 base,
# entropy 0.066 -> 0.020, 2026-09-05): with 32 samples per prompt the group
# baseline is less noisy and fewer groups degenerate to all-correct/all-wrong
# zero-advantage no-ops on this low-entropy model, at 2x the generation cost.
#
# Knobs shared with the n=16 arm (taken from the Qwen3-8B reference arm):
#   * train_prompt_bsz      128           (prompts generated per rollout step)
#   * train_prompt_mini_bsz 32            (prompts per optimizer step)
#   * clip_ratio*           0.2/0.2/0.2/3.0 (verl actor.yaml defaults: symmetric
#                                          band [0.8, 1.2], dual-clip c=3.0; NOT
#                                          DAPO's clip-higher 0.28)
#   * weight_decay          0.01          (verl actor.optim default)
#
# GEOMETRY. train_batch_size = 128 prompts x 32 samples = 4096 sequences
# generated per rollout step; ppo_mini_batch_size = 32 prompts = 1024 sequences
# per optimizer step; ppo_epochs = 1 -> FOUR gradient updates per rollout step
# (the same 4 updates/step as the n=16 arm, each over twice as many sequences).
# Consequences:
...
#     starts at ~0.06.
#   * one weight sync + one old_log_prob pass (4096 seqs) per 4 updates.
#   * 47,981 prompts / 128 = 374 rollout steps = 1,496 optimizer updates per
#     epoch. trainer.test_freq / save_freq count ROLLOUT steps. 1024 seqs per
#     update / 8 DP ranks = 128 per rank (divisible).
...
#   * GRPO group advantages (R - mean) / std over the 32 samples; all-correct/
...
#
# MEMORY. Unchanged from the n=16 arm: gpu_memory_utilization=0.5 is the
...
# step 1). Doubling n only changes how many KV fills generation takes
# (4096 / 8 = 512 sequences per GPU per step vs 256), not the peak: the
# mini-batch grows to 1024 sequences but micro-batch 1 with gradient
# accumulation keeps the trainer footprint the same. Expect roughly 2x the
# n=16 arm's step time (that arm measured ~2 min generation + ~2 min update
...
# 128 prompts x 32 rollouts = 4096 seqs generated per step; 32 prompts x 32 =
# 1024 seqs per optimizer step; ppo_epochs=1 -> 4 gradient updates per step.
# (Batch geometry from the Qwen3-8B B128xn16 mini32 reference arm; n doubled.)
