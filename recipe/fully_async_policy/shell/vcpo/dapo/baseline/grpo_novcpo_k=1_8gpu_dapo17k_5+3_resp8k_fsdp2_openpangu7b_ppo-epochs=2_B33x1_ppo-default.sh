...
#     aime-2024 106/158/261, aime-2025 91/172/795 (min/median/max). Nothing approaches
#     max_prompt_length=2048, so filter_overlong_prompts drops no rows. (A broken chat
#     template would look the same as an empty dataset: rl_dataset swallows the exception
#     and returns max_prompt_length+1 for the row.)
...
#     Checked on a real slow-think shape: a decoy "Answer: 7" inside [unused16]...[unused17]
#     LOSES to the final "Answer: 42" (score 1.0), because the DAPO prompt puts the answer
#     on the last line. [unused16]/[unused17] are not special tokens and DO survive the
#     reward decode, so a delimiter-aware scorer would be possible - deliberately not
#     used, so this arm's reward is identical to the Qwen arms'.
#   * THE ONE LIVE RISK: a verbose epilogue AFTER the answer pushes it out of the 300-char
#     window and scores -1.0 with pred='[INVALID]', same as \boxed{} with no Answer: line.
#     WATCH THE [INVALID] RATE in the first validation.
#
# BOS (data.add_bos_token_to_prompt=True, default since 2026-09-24; runs before that date
# trained WITHOUT BOS). The Pangu tokenizer has add_bos_token=true and a chat template
# that never emits <s>; the official recipe tokenizes the rendered template with a plain
# tokenizer(text) call, so real prompts start with <s> (id 1). verl's default drops it
# (13 vs 14 tokens measured). With the flag on, RLHFDataset (input_ids, raw_prompt_ids,
# the overlong filter) and the single-turn agent loops go through
# verl/utils/dataset/prompt_utils.py::maybe_prepend_bos, exactly as on the Megatron
# openPangu arms. Tagged " bos" in exp_name, so BOS runs never share a log dir with the
# earlier no-BOS runs. add_bos_token_to_prompt=False reproduces those.
#
# VALIDATION SAMPLING (default since 2026-09-24): temperature 0.8 / top-p 0.7 / n=1, the
# same as every other openPangu arm and the Qwen3-8B arms, so val-core curves are
...
#
# Same exact-numerics FSDP2 recipe as the Qwen 5+3 arm: fp32 params (= fp32 master) +
# plain torch AdamW, no offload, no stochastic rounding. openPangu-7B is ~8.0B params
# (34 layers, hidden 4096, FFN 12800, vocab 153376), i.e. the same shape budget as
# Qwen3-8B, so the ~44 GB/GPU estimate above carries over unchanged.
#
# max_response_length stays 8192, as in the Qwen arms, so the two are comparable.
# openPangu's slow-think mode is verbose and the DAPO recipe for a 7B suggests ~20480;
# raising it is a deliberate separate experiment (and costs activation memory).
#
...
bos_tag=""
if [[ "${add_bos_token_to_prompt}" == "True" ]]; then bos_tag=" bos"; fi
# Validation sampling; see VALIDATION SAMPLING in the header.
