...
# grpo_novcpo_k=2_8gpu_dapo17k_5+3_resp8k_megatron_offload_ppo-epochs=2_B33x1_decoupled.sh
#
# ARM B of the pair that reproduces the same-named arm of branch 'rollout-dapo' on
...
# corrected:
#   * A = ..._is-pg.sh: IS-weighted policy gradient, no trust region -- what the
#     source arm EXECUTED (skip_recompute_old_log_prob=True).
#   * B = this file: 3-policy decoupled PPO -- what the source arm DECLARED with
#     algorithm.rollout_correction.bypass_mode=False.
#
# WHAT DECOUPLED MEANS HERE. Three policies are kept apart:
#   pi_rollout  the vLLM bf16 generator, up to k=2 param versions stale
#   pi_old      the training engine at pull time -- async_training.use_rollout_log_probs=False
#               makes the driver spend one extra forward pass to get it
#   pi_theta    the policy being updated, drifting across the 2 PPO epochs
# The two error sources then get separate machinery: the PPO clip on
# pi_theta/pi_old (0.2/0.2, dual-clip 3.0) bounds the drift WITHIN the update, while
# trunc(pi_old/pi_rollout, 2.0) -- computed centrally in the driver and multiplied
# into the vanilla loss -- corrects the rollout-vs-training gap. Cost: one extra full
# forward pass per pull.
#
# LOG SURFACE (differs from arm A -- do not plot the two together blindly):
#   * actor/pg_clipfrac / pg_clipfrac_lower are live here (arm A has no such keys).
#   * actor/ppo_kl is KL(current || old) here, but KL(current || rollout) in arm A.
#   * rollout_corr/* are the central (driver-side) IS statistics against pi_old, plus
#     the actor's current-vs-rollout diagnostics from
#     compute_rollout_corr_metrics_from_logprobs.
#   * actor/entropy comes free from the old-log-prob forward, i.e. it is measured at
#     PULL TIME on pi_old; arm A measures it inside the update. calculate_entropy is
#     therefore left False here -- setting it True would pay a second logits clone.
#
...
# Token-level truncated IS at 2.0 against pi_old, applied on top of the vanilla
# PPO-clip loss -- the decoupled (3-policy) reading of the source script. Note
# bypass_mode/use_policy_gradient are INERT on the fully-async path (their only
# consumer, apply_rollout_correction(), is called from verl/trainer/ppo/ray_trainer.py,
# never from this recipe); what makes this arm decoupled is
# async_training.use_rollout_log_probs=False below, which makes the driver compute a
# real pi_old instead of aliasing it to the rollout log-probs.
rollout_is="token"
...
# False => the driver recomputes old_log_probs with the training engine (one extra
# forward pass per pull), which is what makes the IS weights and the PPO clip live.
