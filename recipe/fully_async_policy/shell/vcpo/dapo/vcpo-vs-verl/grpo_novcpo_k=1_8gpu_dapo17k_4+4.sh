...
# identical to vcpo_k=2_8gpu_dapo17k_4+4.sh except the VCPO mechanisms are left
# at their defaults (off): actor.update_policy_per_traj=False,
# actor.ess_scaling.enable=False, actor.grad_baselining.enable=False.
# The actor takes the stock verl update path; rollout machinery (partial rollout,
# queue, bsz_per_dp_rank), IS correction, and all batch/optimizer settings match
# the VCPO baseline, so the pair is a clean VCPO-on/off ablation.
...
# ================= Async Training =================
# k is timing-neutral in this layout (concurrency saturates at 128 prompt-groups
# for any k >= 0); set 2.0 to exactly match the VCPO baseline's staleness gating.
...
# ================= Logging =================
exp_name="GRPO-noVCPO k-${staleness_threshold} DAPO17K-AIME24 Qwen3-8B ${n_gpus_rollout}-${n_gpus_training} B-${train_prompt_mini_bsz} ${loss_agg_mode} ${max_response_length}-len ${weight_decay}-wd"
