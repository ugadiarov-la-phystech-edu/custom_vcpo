...
# grpo_novcpo_k=2_8gpu_dapo17k_4+4_resp8k_megatron_offload_ppo-epochs=2_B33x1.sh
# 4+4-layout variant of the 5+3 B-33x1 arm
# (grpo_novcpo_k=2_..._5+3_..._ppo-epochs=2_B33x1.sh) with ONE change:
#   * 4 rollout + 4 trainer GPUs (was 5+3): one engine less generating,
#     one DP rank more training. Trainer stays tp=1 pure DP; 33*16=528 seqs
#     divide by DP=4 (528/4=132), so the mini-batch size is unchanged and
#     the arm stays directly comparable to the 5+3 B-33x1 run.
# Everything else inherited from the 5+3 B-33x1 arm:
#   * async_training.require_batches=1: ONE 33-group mini-batch per trainer
#     step (B-33x1); with ppo_epochs=2 that is round(2*1)=2 AdamW updates per
#     step — two shuffled passes over the same 33 groups. Model versions tick
#     per 33-group step.
#   * async_training.staleness_threshold=2 (the VCPO baseline's k=2 gating).
#   * total_rollout_steps=66000 explicit: same generation budget as the
#     B-33x4 arms, up to ~2000 trainer steps of 33 groups.
...
# needs TP>1), HDO full CPU offload with bf16 master weights (do NOT swap for
