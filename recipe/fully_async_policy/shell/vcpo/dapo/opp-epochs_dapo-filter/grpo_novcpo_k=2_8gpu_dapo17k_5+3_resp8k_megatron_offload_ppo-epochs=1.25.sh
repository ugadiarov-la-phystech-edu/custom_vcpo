...
# grpo_novcpo_k=2_8gpu_dapo17k_5+3_resp8k_megatron_offload_opp-epochs_dapo-
# filter.sh (5+3, Megatron + HDO, B-33x4) with the elastic mechanisms OFF and
# a fractional scheduled update instead: ppo_epochs=1.25 keeps the pull at
# 33*4=132 groups (rollouter identical to the B-132 baseline) but runs
# round(1.25*4)=5 AdamW updates — one full shuffled pass plus one reshuffled
# mini-batch replayed. Ablation isolating update count at fixed generation
# throughput and staleness. Stop-the-world validation/saves kept.
...
# Elastic mechanisms disabled in this ablation arm; stop-the-world
# validation/saves keep the clock comparable to a no-validation-no-save run.
