...
# grpo_novcpo_k=10_8gpu_dapo17k_5+3_resp8k_megatron_offload_ppo-epochs=2.sh
# Same as the k=2 arm (grpo_novcpo_k=2_..._ppo-epochs=2.sh) with ONE change:
# async_training.staleness_threshold=10 (see the Async Training section).
...
#   * ppo_epochs=2 (async_training.ppo_epochs): per trainer step the pull
#     is still 33*4=132 groups (rollouter identical to the B-132 baseline),
#     and the trainer runs round(2*4)=8 AdamW updates: two full shuffled
#     passes over the 4 group-complete 33-group mini-batches (every group
#     trained on twice, reshuffled between epochs). Extends the
#     update-count sweep (1.25 -> 1.5 -> 2) at fixed generation throughput
#     and staleness; the second epoch's IS ratios are recomputed against the
#     current policy per update (skip_recompute path).
...
# ================= Async Training =================
# k=10: a much looser staleness gate than the k=2 arm — the rollouter is
# licensed to generate up to (10+1) trainer batches ahead, so generation
# never throttles on the trainer and samples can be up to 10 param versions
# stale at training time (deeper off-policy regime than k=2, which matched
# the VCPO baseline's gating).
staleness_threshold=${staleness_threshold:-10.0}
...
# round(ppo_epochs * require_batches) = 8 driver-side AdamW updates per trainer
# step: two full shuffled epochs over the 4 group-complete 33-group mini-batches
# of the 132-group pull (reshuffled between epochs).
