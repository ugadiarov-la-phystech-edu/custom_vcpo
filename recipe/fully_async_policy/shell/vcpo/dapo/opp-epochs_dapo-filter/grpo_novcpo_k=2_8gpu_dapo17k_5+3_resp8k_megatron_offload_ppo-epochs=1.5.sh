...
#   * ppo_epochs=1.5 (async_training.ppo_epochs): per trainer step the pull
#     is still 33*4=132 groups (rollouter identical to the B-132 baseline),
#     and the trainer runs round(1.5*4)=6 AdamW updates: one full shuffled
#     pass over the 4 group-complete 33-group mini-batches, then two extra
#     updates on reshuffled mini-batches (half the data replayed).
...
# round(ppo_epochs * require_batches) = 6 driver-side AdamW updates per trainer
# step: one full shuffled epoch over the 4 group-complete 33-group mini-batches
# of the 132-group pull, plus two extra updates on reshuffled mini-batches.
ppo_epochs=${ppo_epochs:-1.5}
