...
#
# Fastest possible end-to-end exercise of the rollout-PER arm, for checking
...
#   * save_freq=1 -> one checkpoint per param version, and
#     max_actor_ckpt_to_keep=null (the arm default) keeps them all;
...
#     and a large lr give a real, visible update (the arm sets calculate_entropy=True).
#     This is a plumbing test, not a learning test: neither value has anything to do
#     with the arm's own settings.
...
#     after the checkpoints exist, so TEST_FILE is pointed at a 2-row parquet
#     this script generates to keep it near-free.)
#
# ROLLOUT-PER SPECIFIC: with every group zero-variance (all responses truncated
# and scored the same), each step takes the replay mode's degenerate path -
# train on the original batch, buffer stays empty, replay/degenerate_step=1 -
# so this smoke also exercises that fallback. ppo_epochs stays at the arm's
# default (1); the baseline smoke's ppo_epochs=2 export is deliberately absent.
#
...
# by rank 0 after an all-ranks gather). Expect ~50 GB of checkpoints.
#
...
# model, weights actually changed between the checkpoints, timing_state.json
...
ARM_SCRIPT=${ARM_SCRIPT:-"${HERE}/grpo_rollout-per_8gpu_dapo17k_5+3_resp8k_megatron_offload_B33x1.sh"}
...
export lr=${lr:-1e-4}           # 100x the arm's, so the updates clear bf16 rounding
export exp_name
