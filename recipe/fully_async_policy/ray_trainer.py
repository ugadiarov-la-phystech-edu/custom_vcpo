...
class FullyAsyncRayPPOTrainer(RayPPOTrainer):
    ...
    def _update_actor_fractional_epochs(self, batch, ppo_epochs: float, metrics, timing_raw):
        ...
        epochs over the pulled batch (``async_training.ppo_epochs``).

        The batch is split into ``require_batches`` group-complete mini-batches
        in a shuffled group order (reshuffled every epoch), and
        ``max(1, round(ppo_epochs * require_batches))`` mini-batch updates are
        run driver-side, one ``update_actor`` call (= one optimizer step) per
        mini-batch. With require_batches=4: ppo_epochs=0.25 trains on a single
        random mini-batch (a quarter of the pull), 1.0 covers the whole batch
        once in shuffled order, and 1.5 covers it once then replays a
        reshuffled half. Metrics are averaged across the updates under the
        usual unprefixed keys. Falls back to the vanilla whole-batch call if
        the batch cannot be split into equal group-complete mini-batches.
        """
