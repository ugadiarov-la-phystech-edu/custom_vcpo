...
class TestWiring:
    ...
    def test_no_arm_exports_the_knob(self):
        ...
        emulation in a script that is supposed to run un-capped on real H100s. Machine-specific
        LAUNCHERS (run_*.sh, e.g. the remote_h200 ones) are the place that exports it, so they are
        excluded here."""
        shell = os.path.join(REPO_ROOT, "recipe/fully_async_policy/shell")
        scripts = [
            p
            for p in glob.glob(os.path.join(shell, "**", "*.sh"), recursive=True)
            if not os.path.basename(p).startswith("run_")
        ]
        assert scripts
        for path in scripts:
            with open(path) as f:
