...
compute_global_ess_from_log_weights replaces raw-space ESS pipelines whose
range limits censored the brake signal at both ends (observed in the 2026-08
replay runs):

- a dominant sequence with log-weight far above 0 overflows exp/sums in
  fp32 -> ESS reads 0.0/NaN -> the brake scales the LR toward zero and the
  update is silently skipped;
- batches with all log-weights <~ -87 flush every weight to zero.
...
class TestRangeRegressions:
    def test_overflow_dominant_weight_reads_one_over_n_not_zero(self):
        """Regression for the lr=0 steps: one log-weight at +60 made raw-space
        pipelines read ESS = 0.0/NaN. The true value is ESS ~ 1 (total
        single-sequence domination) -> ratio ~ 1/B."""
