...
def compute_policy_loss_cppo(
    ...
    with torch.no_grad():
        ...
        # T is the (padded) response length and t is the 1-based absolute token position.
        # This matches the reference CPPO implementation, which keys the schedule on the
        # fixed padded length rather than each sequence's valid length; responses are
        # right-padded, so padded positions are zeroed by response_mask below.
