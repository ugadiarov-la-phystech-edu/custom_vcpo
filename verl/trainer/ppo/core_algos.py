...
def _warn_once_cppo_clip_ratio_c(value: float) -> None:
    ...
    much more truncated objective. Warn once per process rather than raise: the value is
    legal, just very unlikely to be intended."""
    global _CPPO_CLIP_RATIO_C_WARNED
    if not _CPPO_CLIP_RATIO_C_WARNED:
        _CPPO_CLIP_RATIO_C_WARNED = True
        logger.warning(
            "loss_mode=cppo is running with actor.clip_ratio_c=%s, which is the repo's dual-clip "
            "default rather than a CPPO choice. CPPO uses clip_ratio_c as its truncated-IS cap; "
            "the paper (and the shipped cppo launch script) use 20.0. Set it explicitly.",
            value,
        )
...
def compute_policy_loss_cppo(
    ...
    CPPO (Binary-TV variant) keeps a truncated-IS REINFORCE surrogate but replaces the
    uniform per-token trust-region test with a position-weighted token-level threshold
    and a cumulative prefix budget. Only the masking decision is CPPO-specific; the
    loss term itself is a detached-weight policy gradient.

    See "Beyond Uniform Token-Level Trust Region in LLM Reinforcement Learning"
    (paper/cppo.pdf), Eq. (8)-(11) and Appendix D for the batched mask construction
    this implements. Ported from the reference implementation in the authors' verl
    fork (compute_policy_loss_cppo).

    For a single response of length ``T`` (``T`` is the padded response length and
    ``t`` the 1-based absolute token position, matching the reference implementation):
    ...
    where ``delta_b_min`` is ``cppo.cppo_delta_b``; quantile ``cppo.cppo_delta_b_q`` and
    scale ``cppo.cppo_delta_b_k`` default to ``(0.9, 1.0)`` (the paper's P90 calibration).

    IMPORTANT caller contract (replay pipeline): ``old_log_prob`` must be the BEHAVIOR
    policy's log-probs (mu, the cached rollout log-probs). Passing the ratio-anchored
    ``log_prob.detach()`` makes ratio == 1 and D_t == 0 everywhere, so the mask never
    binds and the loss degenerates to unweighted REINFORCE.
    ...
    Args:
        ...
        rollout_is_weights (torch.Tensor | None):
            Optional extra importance weights, shape (batch_size, response_length).
            The replay pipeline passes None: the loss's own truncated ratio already
            plays that role against mu.
    """
    ...
    cppo = pl.cppo
    # Like DPPO, the token-level divergence threshold delta is read from clip_ratio.
    # CAVEAT on the defaults: the paper's delta = 0.15 / delta_b_min = 0.02 (Table 3) were
    # calibrated on its Top-K (K=20) reduced-TV estimate of D_t, while this implementation
    # (like upstream) uses Binary-TV. Binary-TV is a lower bound on the Top-K reduced TV, so
    # transplanting those thresholds makes both the token-level and the prefix constraint
    # looser than the paper's runs. See CPPO_PORT_PARAMS_AND_CAVEATS_DISCUSSION.md.
    ...
    # Quantile and scale of the budget calibration; (0.9, 1.0) reproduces the paper's P90.
    delta_b_q = float(cppo.get("cppo_delta_b_q", 0.9))
    delta_b_k = float(cppo.get("cppo_delta_b_k", 1.0))
    # Upper bound of the budget clamp, as a multiple of delta_b_min. The paper's Eq. 22
    # uses 2; the reference implementation tunes it to 5, which is the default here.
    delta_b_max_mult = float(cppo.get("cppo_delta_b_max_mult", 5.0))
    # Which T the position schedule spans: each row's own valid length ("sequence",
    # the paper's Eq. 9) or the padded response width ("padded", the reference
    # implementation). See CPPO_PORT_PARAMS_AND_CAVEATS_DISCUSSION.md.
    w_len_mode = str(cppo.get("cppo_w_len_mode", "sequence"))
    ...
    assert delta_b_max_mult >= 1.0, (
        f"cppo.cppo_delta_b_max_mult must be >= 1 (it is the clamp ceiling in units of "
        f"cppo_delta_b); got {delta_b_max_mult}."
    )
    assert w_len_mode in ("sequence", "padded"), (
        f"cppo.cppo_w_len_mode must be 'sequence' or 'padded'; got {w_len_mode!r}."
    )
    ...
    # NOTE: clip_ratio_c is a real ActorConfig field whose repo default is the dual-clip
    # constant 3.0, so the 20.0 fallback below is NOT what an unconfigured run gets — a
    # script that leaves clip_ratio_c alone truncates 6.7x tighter than the paper's cap.
    clip_ratio_c = config.get("clip_ratio_c", 20.0)
    if float(clip_ratio_c) == 3.0:
        _warn_once_cppo_clip_ratio_c(clip_ratio_c)
    ...
    with torch.no_grad():
        ...
        # Binary-TV per-token divergence D_t = |pi(y_t) - mu(y_t)|.
        ...
        # t is the 1-based absolute token position; T is either the row's own valid
        # length ("sequence", the paper) or the padded response width ("padded", the
        # reference implementation). The two agree only for a full-length row: with
        # T_padded >> T_seq the schedule barely leaves 1, so the position mechanism goes
        # inert for short responses (CPPO_PORT_PARAMS_AND_CAVEATS_DISCUSSION.md).
        # Padded positions are zeroed by response_mask either way.
        bs, resp_len = response_mask_f.shape
        pos = torch.arange(1, resp_len + 1, device=log_prob.device).unsqueeze(0).expand(bs, -1).float()
        if w_len_mode == "sequence":
            T = response_mask_f.sum(dim=-1, keepdim=True)  # (B, 1), each row's valid length
        else:
            T = torch.full((bs, 1), float(resp_len), device=log_prob.device, dtype=response_mask_f.dtype)
        # Written in Algorithm 1's own form rather than the algebraically equivalent
        # w_min + (1 - w_min)(T - t)/(T - 1): the numerator (t - 1) is 0 at the first
        # token, so w_1 = 1 for ANY T, including a single-token response (where the
        # rearranged form would hand out w_min, the LOOSEST weight, instead of the
        # tightest). clamp_min(1) only keeps the denominator finite in that case.
        elapsed = ((pos - 1.0) / (T - 1.0).clamp_min(1.0)).clamp(0.0, 1.0)
        w_t = (1.0 - (1.0 - w_min) * elapsed) * response_mask_f
        ...
        #   delta_b^seq = clamp(delta_b_k * quantile(D, delta_b_q),
        #                       delta_b, delta_b_max_mult * delta_b).
        D_for_q = torch.where(response_mask_f.bool(), D_t, torch.full_like(D_t, float("nan")))
        q_seq = torch.nanquantile(D_for_q.float(), q=delta_b_q, dim=-1)  # (B,)
        # Empty sequence -> NaN -> fall back to the floor.
        q_seq = torch.nan_to_num(q_seq, nan=delta_b)
        delta_b_seq = (delta_b_k * q_seq).clamp(min=delta_b, max=delta_b_max_mult * delta_b).unsqueeze(-1)
    ...
    # toward-mu tokens), matching DPPO's pg_clipfrac semantics; plus the fraction kept
    # purely by the toward-mu clause (drift-correction pressure indicator).
    ...
    toward_mu_frac = verl_F.masked_mean(toward_mu.float(), response_mask_f)
