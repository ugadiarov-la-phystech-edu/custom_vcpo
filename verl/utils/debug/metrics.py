...
def rollout_actor_probs_pearson_corr(
    ...
    """Pearson correlation between exp(policy log-probs) and exp(rollout
    log-probs) over valid response tokens (https://arxiv.org/pdf/2506.13585).

    Robust wrapper around :func:`pearson_correlation_coefficient` for
    per-micro-batch logging from the deferred-correction path
    (``algorithm.rollout_correction.log_probs_pearson_corr``): shape
    mismatches, fewer than two valid tokens, and degenerate (zero-variance)
    inputs return 0.0 instead of raising or producing NaN.
    """
